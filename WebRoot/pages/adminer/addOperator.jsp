 ...
 <div id='allmap' style='width: 50%; height: 70%; left:450px; top:30px;z-index:5; position: absolute; display: none'></div>
	...
	<form action="" method="post" class="form form-horizontal" id="form-article-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="password" class="input w50" value="" maxlength='20' placeholder="请再次输入密码(必填)" id="operatorPassword_confirm" name="operatorPassword_confirm">
...
function addOperator(){
			...
			$.post("addOperator.do",{operatorName:operatorName,operatorPassword:operatorPassword,operatorUuid:operatorUuid,operatorMajor:operatorMajor,operatorPhone:operatorPhone,operatorAddress:operatorAddress,roleId:roleId},
				function(data){
					if(data==1){
						Showbo.Msg.alert("运营商添加成功!",function (){window.location='searchOperator.do?userId=<%=rb.getUserId()%>&page=1&roleName=<%=rb.getRoleName()%>';});
						
					}else if(data==-1){
						document.getElementById("name_notice").innerHTML="*运营商名称已存在,请重新填写!";
						document.getElementById("operatorName").focus();
					}else if(data==-2){
						document.getElementById("uuid_notice").innerHTML="*UUID已存在,请重新填写!";
						document.getElementById("operatorUuid").focus();
					}else if(data==-3){
						document.getElementById("major_notice").innerHTML="*major已存在,请重新填写!";
						document.getElementById("operatorMajor").focus();
					}else{
						Showbo.Msg.alert("运营商添加失败!");
				}
			});
		}
</script> 
