...
<div class="page-container">
    ...
    <form action="" method="post" class="form form-horizontal" id="form-article-add">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <select name="selectDriver" id="selectDriver" class="input" style="width:200px; line-height:17px;">
                    ...
                    <c:forEach var="list" items="${dbList}">
                        <option value="${list.dname}" <c:if test="${list.dname.equals(ubl.dname)}"><c:out value="selected"/></c:if>>${list.dname}</option>
                    </c:forEach>
        ...
        <input type="hidden" value="${ubl.id}" name="id" id="id"/>

        <div class="row cl">
            <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
                <button onClick="updateDispatch();" class="btn btn-primary radius" type="button"><i class="Hui-iconfont">&#xe632;</i>确认</button>
...
<script type="text/javascript">
        ...
        $.post("updateDispatch.do", {
            ...
            function(data) {
                if (data == 1) {
                    Showbo.Msg.alert("更换司机成功!",function(){window.location='dispatchDriver.do?page=1';});

                } else {
                    Showbo.Msg.alert("更换司机失败!");
                }
            });
    }
