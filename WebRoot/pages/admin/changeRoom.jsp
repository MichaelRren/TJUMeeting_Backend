...
<div class="page-container">
    ...
    <form action="" method="post" class="form form-horizontal" id="form-article-add">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <select name="selectHotel" id="selectHotel" class="input" style="width:200px; line-height:17px;">
                    <sapn id = "select_hotel" class="c-red">
                    <c:forEach var="hbList" items="${hbList}">
                        <option value="${hbList.hname}" <c:if test="${ub.hname.equals(hbList.hname)}"><c:out value="selected"/></c:if>>${hbList.hname}</option>
                    </c:forEach>
                    </sapn>
                </select>
            </div>
        ...
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>分配房间：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <select name="selectRoom" id="selectRoom" class="input" style="width:200px; line-height:17px;">
                    <span id="select_room" class="c-red">
                    <c:forEach var="htList" items="${htList}">
                        <option value="${htList}" <c:if test="${ub.htype.equals(htList)}"><c:out value="selected"/></c:if>>${htList}</option>
                    </c:forEach>
                    </span>
                </select>
        ...
        <div class="row cl">
            <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
                <button onClick="changeRoom();" class="btn btn-primary radius" type="button"><i class="Hui-iconfont">&#xe632;</i>确认</button>
...
<script type="text/javascript">
    $(function(){
        ...
        function changeRoom(){
            ...
            if(hname==""){
                document.getElementById("select_hotel").innerHTML="*请选择酒店!";
            ...
            function(data) {
                if (data == 1) {
                    Showbo.Msg.alert("更改房间成功!",function(){window.location='viewDispatchRoom.do?page=1';});

                } else {
                    Showbo.Msg.alert("更改房间失败!");
                }
            });
    }
